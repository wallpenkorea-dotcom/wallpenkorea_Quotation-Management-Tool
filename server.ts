...
async function startServer() {
  ...
  app.get('/api/settings/banner', (req: Request, res: Response) => {
    try {
      // 1. Check if user explicitly set a banner in settings.json
      const settingsPath = path.join(DATA_DIR, 'settings.json');
      if (fs.existsSync(settingsPath)) {
      ...
      // 2. Check for original PNG banner in public directory
      const publicDir = path.join(process.cwd(), 'public');
      if (fs.existsSync(publicDir)) {
        if (fs.existsSync(path.join(publicDir, 'wallpen-banner.png'))) {
          return res.json({ bannerUrl: '/wallpen-banner.png' });
        }
        if (fs.existsSync(path.join(publicDir, 'wallpen-banner1.png'))) {
          return res.json({ bannerUrl: '/wallpen-banner1.png' });
        }
      }

      res.json({ bannerUrl: '/wallpen-banner.png' });
    } catch {
      res.json({ bannerUrl: '/wallpen-banner.png' });
    }
  });
